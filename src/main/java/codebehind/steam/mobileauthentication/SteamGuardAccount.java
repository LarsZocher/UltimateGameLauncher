...
public class SteamGuardAccount {
	...
	public boolean DeactivateAuthenticator(int scheme) {
		...
		postData.add("steamid", this.session.getSteamID()+"");
		postData.add("steamguard_scheme", scheme+"");
		postData.add("revocation_code", this.config.revocationCode);
		postData.add("access_token", this.session.getOAuthToken());
		
		try
		{
			String response = SteamWeb.MobileLoginRequest(APIEndpoints.STEAMAPI_BASE + "/ITwoFactorService/RemoveAuthenticator/v0001", "POST", postData, null, null);
			RemoveAuthenticatorResponse removeResponse = JsonHelper.Deserialize(RemoveAuthenticatorResponse.class, response);
    ...
    public Confirmation[] FetchConfirmations() throws Throwable {
	    ...
	    List<Confirmation> confs = new ArrayList<>();
        for(int i = 1; i<response.split("<div class=\"mobileconf_list_entry\" ").length; i++){
	        String confirmationData = response.split("<div class=\"mobileconf_list_entry\" ")[i];
        	Confirmation conf = new Confirmation();
        	conf.setID(confirmationData.split("data-confid=\"")[1].split("\"")[0]);
	        conf.setKey(confirmationData.split("data-key=\"")[1].split("\"")[0]);
	        conf.setCancel(confirmationData.split("data-cancel=\"")[1].split("\"")[0]);
	        conf.setAccept(confirmationData.split("data-accept=\"")[1].split("\"")[0]);
	        conf.setType(Integer.valueOf(confirmationData.split("data-type=\"")[1].split("\"")[0]));
	        switch(conf.getType()){
		        case 1:{
			        conf.setPicture(confirmationData.split("<img src=\"")[1].split(", ")[1].split(" 2x")[0]);
		        	break;
		        }
		        case 2:{
			        conf.setPicture(confirmationData.split("<img src=\"")[1].split(", ")[1].split(" 2x")[0]);
		        ...
		        case 3:{
			        conf.setBorderColor(confirmationData.split("border-color: ")[1].split(";")[0]);
			        conf.setPicture(confirmationData.split("<img src=\"")[1].split(", ")[1].split(" 2x")[0]);
			        break;
		        }
	        }
	        String descData = confirmationData.split("<div class=\"mobileconf_list_entry_description\">")[1];
	        conf.setDescription(descData.split("<div>")[1].split("</div>")[0]);
	        conf.setPrice(descData.split("<div>")[2].split("</div>")[0]);
	        conf.setDate(descData.split("<div>")[3].split("</div>")[0]);
	        confs.add(conf);
        }
	    
	    return confs.toArray(new Confirmation[confs.size()]);
    }
