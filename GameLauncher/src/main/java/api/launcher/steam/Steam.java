...
public class Steam {
	...
	public List<SteamApp> getAppsFromUser(String steam64id, boolean free) {
		...
		try {
			...
			for(int i = 0; i < games.length(); i++) {
				...
				game.setWebData(games.getJSONObject(i));
				apps.add(game);
			}
	...
	public SteamApp getSteamApp(String appid){
		List<String> appids = new ArrayList<>();
		appids.add(appid);
		return getSteamApps(appids).get(0);
	}
	
	public List<SteamApp> getSteamApps(List<String> appids){
		if(disabled||appids.size()==0) return new ArrayList<>();
